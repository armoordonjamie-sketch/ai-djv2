...
def get_mix_start_in_song(song_duration: float) -> float:
...
def create_intro_mix(
    ...
    if tts_path and os.path.exists(tts_path):
        ...
        tts_faded = tts_norm.filter('afade', t='out', st=tts_duration - 0.5, d=0.5)
        song_faded = song_delayed.filter('afade', t='in', st=0, d=overlap_duration)
        
        final_audio = ffmpeg.filter([tts_faded, song_faded], 'amix', inputs=2, duration='longest', dropout_transition=0)
    else:
...
def create_dj_mix(
    ...
    song2_duration = get_duration(song2_path) - song2_start_sec
    tts_duration = get_duration(tts_path) if tts_path and os.path.exists(tts_path) else 0.0
    ...
    song1_start = get_mix_start_in_song(song1_duration)
    ...
    # Delay Song B to start at transition
    delay_seconds = max(segment_transition_pos - (crossfade_duration / 2), 0)
