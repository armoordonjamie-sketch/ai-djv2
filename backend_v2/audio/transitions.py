...
def apply_crossfade(a1, a2, duration: float, peak_time: float, **kwargs):
    """Standard linear crossfade using amix and afade."""
    fade_start = peak_time - (duration / 2)
    a1_v = a1.filter("afade", t="out", st=fade_start, d=duration)
    a2_v = a2.filter("afade", t="in", st=fade_start, d=duration)
...
def apply_quick_cut(a1, a2, duration: float, peak_time: float, **kwargs):
    ...
    hard_cut_duration = 0.1
    a1_cut = a1.filter("afade", t="out", st=peak_time - hard_cut_duration, d=hard_cut_duration)
    a2_cut = a2.filter("afade", t="in", st=peak_time - hard_cut_duration, d=hard_cut_duration)
...
def apply_eq_blend(a1, a2, duration: float, peak_time: float, **kwargs):
    ...
    fade_len = duration * 0.25

    a1_fade_out_start = fade_end - fade_len
    a1_vol_expr = (
        f"if(lt(t,{a1_fade_out_start}),1,if(lt(t,{fade_end}),"
        f"1-((t-{a1_fade_out_start})/{fade_len}),0))"
    )

    a2_fade_in_end = fade_start + fade_len
    a2_vol_expr = (
        f"if(lt(t,{fade_start}),0,if(lt(t,{a2_fade_in_end}),"
        f"(t-{fade_start})/{fade_len},1))"
    )

    a1_faded = a1_swept.filter("volume", volume=a1_vol_expr, eval="frame")
    a2_faded = a2_swept.filter("volume", volume=a2_vol_expr, eval="frame")
...
def apply_bass_swap(a1, a2, duration: float, peak_time: float, **kwargs):
    ...
    w1_high = f"if(lt(t,{fade_start}),1,if(lt(t,{fade_end}),1-((t-{fade_start})/{duration}),0))"
    w2_high = f"if(lt(t,{fade_start}),0,if(lt(t,{fade_end}),(t-{fade_start})/{duration},1))"
...
def apply_filter_sweep(a1, a2, duration: float, peak_time: float, **kwargs):
    ...
    a1_faded = a1_swept.filter("volume", volume=a1_vol_expr, eval="frame")
    a2_faded = a2.filter("volume", volume=a2_vol_expr, eval="frame")
...
def apply_echo_out(a1, a2, duration: float, peak_time: float, **kwargs):
    ...
    a1_vol_expr = f"if(lt(t,{fade_start}),1,if(lt(t,{fade_end}),1-((t-{fade_start})/{duration}),0))"
    a2_fade_in_end = fade_start + fade_len
    a2_vol_expr = f"if(lt(t,{fade_start}),0,if(lt(t,{a2_fade_in_end}),(t-{fade_start})/{fade_len},1))"

    a1_faded = a1_echo.filter("volume", volume=a1_vol_expr, eval="frame")
    a2_faded = a2.filter("volume", volume=a2_vol_expr, eval="frame")
...
def apply_drop_mix(a1, a2, duration: float, peak_time: float, **kwargs):
    ...
    fade_start = peak_time - (duration / 2)
    a1_faded = a1_no_bass.filter("afade", t="out", st=fade_start, d=duration * 0.5)
    a2_faded = a2.filter("afade", t="in", st=fade_start, d=duration * 0.3)
