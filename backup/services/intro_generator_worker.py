...
class IntroGeneratorWorker:
    ...
    async def generate_for_user(self, user_id: str):
        """Generate intros for all moods of a specific user.
        ...
        try:
            async with get_db_session() as db:
                ...
                logger.info(f"Generating intros for user {user_id} ({len(moods)} moods)")
                
                for mood in moods:
                    if mood.id not in self._processing_mood_ids:
                        # Don't await - fire and forget
                        asyncio.create_task(
                            self._generate_intro_for_mood(mood.id, user_id)
                        )
