...
class YouTubeProvider(AcquisitionProvider):
    ...
    async def acquire(
        ...
        try:
            ...
            # Download with timeout - pass db session to prevent nested session deadlock
            result = await asyncio.wait_for(
                downloader.download_song(
                    ...
                    db=db  # Pass the session to avoid nested session deadlock
                ),
                timeout=timeout
            )
            ...
            # Retry with cleaned title if original failed and contained parentheses
            if not result and '(' in intent.title:
                clean_title = intent.title.split('(')[0].strip()
                if clean_title and clean_title != intent.title:
                    retry_query = f"{intent.artist} - {clean_title} official audio"
                    logger.info(f"Retrying download with clean query: {retry_query}")
                    ...
                    try:
                        result = await asyncio.wait_for(
                            downloader.download_song(
                                query=retry_query,
                                artist=intent.artist,
                                title=clean_title, # Store with clean title? Maybe better to keep original intent title in DB metadata? 
                                # Actually, better to pass clean title so file metadata is clean, 
                                # but keep intent.title in DB intent. 
                                # download_song stores in Song table. 
                                # Let's pass clean title to simple download.
                                skip_db_storage=False,
                                db=db
                            ),
                            timeout=timeout
                        ...
                        if result and result.get('uuid'):
                            logger.info(f"Successfully downloaded with clean title: {intent.artist} - {clean_title} (uuid={result['uuid']})")
                            return result['uuid']
                    except Exception as retry_err:
                        logger.warning(f"Retry download failed: {retry_err}")
