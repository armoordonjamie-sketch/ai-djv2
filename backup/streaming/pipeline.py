...
PCM_MAX_LEAD_SEC = 10.0  # Allow feeding up to 10 seconds ahead for better buffering
...
class UserRadioPipeline:
    ...
    def __init__(self, user_id: str, session_id: str):
        ...
        self._now_playing_task: Optional[asyncio.Task] = None
        self._now_playing_segment_path: Optional[str] = None
    ...
    async def stop(self):
        ...
        # Trigger mood training from session feedback (runs in background)
        try:
            from backend_v2.services.mood_enrichment import train_mood_from_session
            ...
            async def run_training():
                async with get_db_session() as db:
                    # Get session to find mood_id
                    result = await db.execute(
                        select(Session).where(Session.session_id == self.session_id)
                    )
                    session = result.scalar_one_or_none()
                    if session and session.mood_id:
                        await train_mood_from_session(
                            session_id=self.session_id,
                            user_id=self.user_id,
                            mood_id=session.mood_id,
                        )
                        logger.info(f"🎓 Triggered mood training for stopped session {self.session_id[:8]}")
            
            asyncio.create_task(run_training())
        except Exception as e:
            logger.warning(f"Failed to trigger mood training on stop: {e}")
    ...
    async def _segment_feeder(self):
        ...
        async def emit_now_playing_for_song(song_meta: Dict[str, Any]) -> None:
            """Emit now_playing and update session state for the active song."""
            ...
            logger.info(f"ÐYZæ Now playing: {song_meta.get('artist')} - {song_meta.get('title')}")
            
            # Update session record with current song (for resume functionality)
            try:
                ...
                async with async_session_factory() as db:
                    await db.execute(
                        ...
                        .values(
                            current_song_uuid=song_meta.get("song_uuid"),
                            current_song_title=song_meta.get("title"),
                            current_song_artist=song_meta.get("artist"),
                            current_song_artwork=song_meta.get("artwork_url"),
                        )
                    )
                    await db.commit()
                    logger.debug(f"Updated session {self.session_id} with current song")
            except Exception as e:
                logger.warning(f"Failed to update session with current song: {e}")
        ...
        while not has_started and not self._shutdown:
            try:
                ...
                if STREAM_STARTUP_MODE == "defer":
                    ...
                    await asyncio.sleep(0.05)  # Check frequently for fast response
                else:
        ...
        # For the first segment, feed a small amount of silence before the actual segment
        # Note: The intro mix file itself also contains 2.5s of leading silence, so total is ~3.5s
        # This ensures the frontend player has time to start playback before intro TTS plays
        # The "Ready to play!" status is emitted, but the frontend needs time to call play()
        logger.info("🎧 First segment ready, feeding brief silence to allow frontend player to start...")
        try:
            await self._feed_silence(1.0)  # Feed 1 second of silence before first segment (mix file has 2.5s built-in)
        except Exception as e:
            logger.warning(f"Failed to feed startup silence: {e}")
        ...
        while not self._shutdown:
            ...
            # Update session for ALL segments that have a song_uuid, even if they are intros (like initial mix)
            if song_meta and song_meta.get("song_uuid"):
                now_playing_delay = song_meta.get("now_playing_offset_sec", 0)
                if not isinstance(now_playing_delay, (int, float)):
                    now_playing_delay = 0

                if self._now_playing_task and not self._now_playing_task.done():
                    self._now_playing_task.cancel()
                self._now_playing_segment_path = segment_path

                if now_playing_delay > 0:
                    logger.info(
                        "Delaying now_playing by %.1fs for transition peak",
                        now_playing_delay,
                    )

                    async def delayed_emit(segment_path_snapshot: str) -> None:
                        try:
                            await asyncio.sleep(now_playing_delay)
                        except asyncio.CancelledError:
                            return
                        if self._shutdown:
                            return
                        if self._now_playing_segment_path != segment_path_snapshot:
                            return
                        await emit_now_playing_for_song(song_meta)

                    self._now_playing_task = asyncio.create_task(
                        delayed_emit(segment_path)
                    )
                else:
                    await emit_now_playing_for_song(song_meta)

            elif song_meta and song_meta.get("is_intro"):
                # Pre-generated intro with song metadata - emit now_playing and update session
                logger.info(f"🎵 Playing pre-generated intro: {song_meta.get('artist')} - {song_meta.get('title')}")
                ...
                try:
                    ...
                    async with async_session_factory() as db:
                        ...
                        await db.commit()
                        logger.debug(f"Updated session {self.session_id} with intro song")
                except Exception as e:
                    logger.warning(f"Failed to update session with intro song: {e}")
                
            # Clear skip flag before feeding new segment
            self._skip_requested = False
            
            # Get start offset for resume support
            start_offset_sec = song_meta.get("start_offset_sec", 0) if song_meta else 0
            
            # Decode segment to PCM and feed to encoder
            if start_offset_sec > 0:
                logger.info(f"🎵 Feeding segment: {segment_path} (starting at {start_offset_sec}s)")
            else:
                logger.info(f"🎵 Feeding segment: {segment_path}")
            
            try:
                await self._decode_and_feed_segment(segment_path, start_offset_sec=start_offset_sec)
            except Exception as e:
    ...
    async def _decode_and_feed_segment(self, segment_path: str, start_offset_sec: float = 0):
        ...
        Args:
            segment_path: Path to the audio file
            start_offset_sec: Offset in seconds to start decoding from (for resume)
        """
        ...
        try:
            # For resume with offset, always use FFmpeg (it can seek)
            if start_offset_sec > 0 or not segment_path.lower().endswith(".wav"):
                bytes_written = await self._feed_ffmpeg_segment(segment_path, start_offset_sec=start_offset_sec)
            else:
    ...
    async def _feed_ffmpeg_segment(self, segment_path: str, start_offset_sec: float = 0) -> int:
        ...
        # Add seek position for resume (before input for fast seeking)
        if start_offset_sec > 0:
            cmd.extend(["-ss", str(start_offset_sec)])
...
async def get_user_pipeline(
    ...
    resume_song_uuid: str = None,
    resume_position_sec: float = None,
    ...
    Args:
        ...
        resume_position_sec: Optional position to resume from (seconds)
    ...
    async with _pipelines_lock:
        ...
        # Check if we need to restart (different session = always restart)
        # NOTE: The stream.py endpoint generates a NEW session_id UUID for every /start call
        # So if session_id differs, it's a new stream request and we must restart
        needs_restart = False
        if existing and existing.is_running:
            existing_session = getattr(existing, 'session_id', None)
            
            # Different session_id means a new /stream/start call - ALWAYS restart
            if session_id and existing_session and session_id != existing_session:
                existing_mood = getattr(existing._dj_loop, 'mood_id', None) if hasattr(existing, '_dj_loop') else None
                logger.info(f"New session requested (old={existing_session[:8]}..., new={session_id[:8]}..., mood: {existing_mood} -> {mood_id}), restarting pipeline")
                needs_restart = True
        ...
        if not create_if_missing and not needs_restart:
            return None
            
        # Clean up old pipeline if exists
        if existing:
            logger.info(f"Stopping existing pipeline for user {user_id}")
            await existing.stop()
