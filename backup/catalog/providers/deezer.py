...
class DeezerCatalogProvider(CatalogProvider):
    ...
    async def get_artist_tracks(
        ...
        """Get tracks by a specific artist using artist search + top tracks.
        
        This is more reliable than generic search which can return
        tribute tracks, covers, or unrelated songs mentioning the artist.
        ...
        Returns:
            List of catalog tracks by the artist
        """
        try:
            # First, find the artist
            artist = await self.client.search_artist(artist_name)
            if not artist:
                logger.debug(f"Artist not found: {artist_name}")
                return []
            
            # Get their top tracks
            tracks = await self.client.get_artist_top_tracks(artist["id"], limit=limit)
            if not tracks:
                logger.debug(f"No top tracks for artist: {artist_name}")
                return []
            
            catalog_tracks = []
            for track in tracks:
                catalog_track = self._convert_top_track(track, artist["name"])
                if catalog_track:
                    catalog_tracks.append(catalog_track)
            
            logger.info(f"Deezer artist '{artist_name}': found {len(catalog_tracks)} top tracks")
            return catalog_tracks
            
        except Exception as e:
            logger.error(f"Deezer artist search error for '{artist_name}': {e}")
            return []
    
    def _convert_top_track(self, track: dict, artist_name: str) -> Optional[CatalogTrack]:
        """Convert Deezer top track response to CatalogTrack.
        
        Top tracks from get_artist_top_tracks have format:
        {
            "id": 123,
            "title": "Song Title",
            "duration": 240,
            "album": {"title": "Album Name", "cover": "url"},
            ...
        }
        """
        try:
            # Extract album title from album dict
            album_data = track.get("album", {})
            album_title = album_data.get("title") if isinstance(album_data, dict) else None
            album_cover = album_data.get("cover") if isinstance(album_data, dict) else None
            
            return CatalogTrack(
                title=track.get("title", "Unknown"),
                artist=artist_name,  # Use the verified artist name
                album=album_title,  # Extract title string, not the whole dict
                provider="deezer",
                provider_id=str(track.get("id", "")),
                isrc=None,  # Not in top tracks response
                duration_ms=track.get("duration", 0) * 1000,
                release_date=None,
                artwork_url=album_cover,
                genres=[],
                popularity=track.get("rank"),  # Use rank as popularity proxy
                features=AudioFeatures(
                    tempo=float(track.get("bpm")) if track.get("bpm") else None,
                )
            )
        except Exception as e:
            logger.error(f"Error converting Deezer top track: {e}")
            return None
