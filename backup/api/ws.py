...
async def websocket_endpoint(
    ...
    # Send welcome message - wrapped in try/catch to handle early disconnects
    try:
        ...
        # Client disconnected before we could send welcome - clean up and exit
        logger.debug(f"WS client disconnected during handshake: user {user_id}")
    ...
    if recent_events:
        ...
        for event in recent_events[-5:]:
            try:
                ...
                # Client disconnected during replay - clean up and exit
                logger.debug(f"WS client disconnected during event replay: user {user_id}")
                await emitter.disconnect(user_id, websocket)
                return
    
    # Send current now_playing if there's an active stream
    # Only re-emit if we have a valid song UUID (skip for intros or pre-start state)
    from backend_v2.streaming.pipeline import get_user_pipeline
    pipeline = await get_user_pipeline(user_id=user_id, session_id=None, create_if_missing=False)
    if pipeline and hasattr(pipeline, '_dj_loop') and pipeline._dj_loop:
        current_song = pipeline._dj_loop.current_song
        if current_song and current_song.get("uuid"):
            logger.info(f"📡 Re-emitting now_playing on WS connect: {current_song.get('artist')} - {current_song.get('title')}")
            try:
                await websocket.send_json({
                    "v": 1,
                    "type": "now_playing",
                    "data": {
                        "song_uuid": current_song.get("uuid", ""),
                ...
                logger.debug(f"WS client disconnected during now_playing re-emit: user {user_id}")
                await emitter.disconnect(user_id, websocket)
                return
