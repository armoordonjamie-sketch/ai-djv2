...
async def enrich_spotify_data_with_ai(spotify_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    try:
        logger.info("Sending request to OpenRouter SDK...")
        ...
        sdk_params = {
            ...
            "stream": False,
            # No session_id for standalone enrichment call
        }
        
        # Call SDK in executor (SDK may be synchronous)
                ...
                with res as event_stream:
                    ...
                    if isinstance(event, dict):
                        if event.get('choices') and len(event['choices']) > 0:
                            return event['choices'][0]['message'].get('content', '')
                    
                    return None
        
        logger.info("Calling OpenRouter SDK...")
        content = await loop.run_in_executor(None, _call_sdk)
        
        if not content:
            logger.error("No content in OpenRouter response")
            return None
        
        logger.info(f"OpenRouter response received: {len(str(content))} chars")
