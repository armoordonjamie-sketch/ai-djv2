...
class UserEventEmitter:
    ...
    async def emit_now_playing(
        ...
        logger.info(f"📡 Emitting now_playing: {artist} - {title} to user {user_id}")
        ...
        logger.info(f"✅ now_playing event emitted for user {user_id}")
    ...
    async def emit_status(
        ...
        should_persist = (
            ...
            step in [StatusStep.ONBOARDING_COMPLETE, StatusStep.FAILED, StatusStep.STOPPED, StatusStep.TRAINING_COMPLETE]
        )
        if should_persist:
