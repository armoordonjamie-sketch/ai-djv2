...
def get_spotify_track_context(
    song: Dict[str, Any],
    bundle: PreferenceBundle
) -> Optional[Dict[str, Any]]:
    """Check if song is from user's Spotify top tracks and return context.
    
    Args:
        song: Song dict with title and artist
        bundle: User's preference bundle with Spotify data
    
    Returns:
        Dict with rank, time_range, title, artist if found, else None
    """
    if not bundle.profile.spotify_connected:
        return None
    
    if not bundle.profile.top_tracks_with_stats:
        return None
    ...
    # Match against Spotify top tracks
    for track in bundle.profile.top_tracks_with_stats:
        ...
        title_match = (track_title in song_title or song_title in track_title)
        artist_match = (track_artist in song_artist or song_artist in track_artist)
...
def _artist_played_too_recently(
    ...
    Args:
        ...
        recent_tracks: Recent history items (typically last 10)
        max_occurrences: Max times artist can appear before rejection (default 2)
        
    Returns:
        True if artist appears more than max_occurrences times
        
    Example:
        - max_occurrences=2: Allow 1-2 plays, reject at 3+
    """
    if not artist:
        return False
    ...
    for item in recent_tracks:
        if not item.artist:
            continue
        if artist_lower in item.artist.lower() or item.artist.lower() in artist_lower:
            count += 1
            if count > max_occurrences:  # Changed from >= to >
                return True
...
async def select_track(
    ...
    if client.enabled:
        ...
        suggestion = await client.generate_song_suggestion(
            bundle=bundle,
            prev_song=prev_song,
            db=db,
        ...
        if suggestion:
            ...
            # NEW: Check if artist appears too frequently in recent history (prevent artist loops)
            elif artist and _artist_played_too_recently(artist, bundle.history.recent_tracks[:10]):
...
async def select_track_via_catalog(
    ...
    NEW FLOW (2024-12):
    1. Search global catalog (Spotify) for mood-matching tracks
    ...
    Args:
        ...
        use_tools: If True, use database tools for context gathering (default False)
    ...
    # Import CatalogTrack at function level (needed for both history conversion and prev_song conversion)
    from backend_v2.catalog.providers import CatalogTrack, AudioFeatures
    
    # Tools will be passed to LLM for tool calling (LLM decides which tools to use)
    # No direct tool calls - let the LLM use tools via tool calling
    ...
    try:
        ...
        # Use session history for exclusion (LLM will use tools to get additional context if needed)
        enhanced_history_ids = set(history_ids) if history_ids else set()
        initial_count = len(enhanced_history_ids)
        logger.info(f"🚫 Excluding {initial_count} songs from current session history")
        
        # Convert ALL history_ids to CatalogTrack format for proper diversity filtering
        # This prevents songs from repeating, even if they were played early in the session
        recent_catalog_tracks = []
        if enhanced_history_ids:
            # Query database for all songs in history with features eagerly loaded
            result = await db.execute(
                select(Song)
                .options(selectinload(Song.features))
                .where(Song.uuid.in_(list(enhanced_history_ids)))
            )
                ...
                if song.artist and song.title:
                    recent_catalog_tracks.append(CatalogTrack(
                        ...
                        features=AudioFeatures(
                            ...
                            tempo=song.features.tempo if song.features else None,
                        )
                    ))
        
        logger.info(f"🚫 Excluding {len(recent_catalog_tracks)} previously played tracks from current session")
        
        # Build exclusion set: only exact matches from current session + DISLIKED TRACKS
        # Removed normalized title exclusion to allow more natural music flow
        excluded_titles = {f"{t.artist.lower()}|{t.title.lower()}" for t in recent_catalog_tracks}
        
        # Add disliked tracks to exclusion list - NEVER suggest these
        if bundle.feedback.recent_dislikes:
            for dislike in bundle.feedback.recent_dislikes:
                if dislike.artist and dislike.title:
                    excluded_titles.add(f"{dislike.artist.lower()}|{dislike.title.lower()}")
                    logger.debug(f"🚫 Excluding disliked track: {dislike.artist} - {dislike.title}")
        
        logger.info(f"🚫 Excluding {len(excluded_titles)} exact matches from current session")
        ...
        # Step 1: Let LLM search Spotify and select track (no pre-search)
        client = get_openrouter_client()
        if not client.enabled:
            logger.warning("⚠️ OpenRouter disabled, falling back to legacy selection")
            metrics.record_local_library_fallback()
            return await select_track(db, bundle, state, prev_song, history_ids)
        
        # Get recent artists for variety filtering
        recent_artists = bundle.history.recent_artists[:10]
        
        # Pass tools to LLM (including search_spotify_catalog)
        tools_to_use = None
        if use_tools:
            from backend_v2.integrations.db_tools import DB_TOOLS
            tools_to_use = DB_TOOLS
            logger.info("🔧 Enabling LLM tool calling for catalog selection (LLM will search Spotify)")
        
        # Call LLM to search Spotify and select track (retry if excluded)
        logger.info("🎯 Asking AI DJ to search Spotify and select track...")
        
        max_reselect_attempts = 3
        rejected_titles = set()
        rejected_artists: Dict[str, str] = {}
        selection = None
        selected_artist = None
        selected_title = None
        rationale = None
        
        for attempt in range(1, max_reselect_attempts + 1):
            selection = await client.generate_track_selection_with_search(
                bundle=bundle,
                prev_song=prev_song,
                excluded_titles=excluded_titles | rejected_titles,
                excluded_normalized_titles=None,  # Removed normalized title exclusion for more natural flow
                recent_artists=recent_artists,
                excluded_artists=list(rejected_artists.values()) if rejected_artists else None,
                db=db,
                agent_name="catalog_selector",
                tools=tools_to_use,
            )
            
            if not selection or not selection.get('parsed'):
                logger.warning("AI selection failed, falling back to legacy selection")
                selection = None
                break
            
            parsed = selection['parsed']
            selected_artist = parsed.get('artist')
            selected_title = parsed.get('title')
            rationale = parsed.get('rationale', 'AI selection')
            
            if not selected_artist or not selected_title:
                logger.warning(f"AI selection missing artist/title: {parsed}, falling back")
                selection = None
                break
            
            selected_artist_lower = selected_artist.strip().lower()
            selected_key = f"{selected_artist_lower}|{selected_title.strip().lower()}"
            if selected_key in excluded_titles or selected_key in rejected_titles:
                logger.warning(
                    f"AI selected excluded track (attempt {attempt}/{max_reselect_attempts}): "
                    f"{selected_artist} - {selected_title}. Retrying."
                )
                rejected_titles.add(selected_key)
                rejected_artists.setdefault(selected_artist_lower, selected_artist.strip())
                selection = None
                selected_artist = None
                selected_title = None
                rationale = None
            ...
            if selected_artist_lower in rejected_artists:
                logger.warning(
                    f"AI selected rejected artist (attempt {attempt}/{max_reselect_attempts}): "
                    f"{selected_artist}. Retrying."
                )
                rejected_titles.add(selected_key)
                selection = None
                selected_artist = None
            ...
            prev_artist = prev_song.get("artist") if prev_song else None
            if prev_artist and prev_artist.strip().lower() == selected_artist_lower:
                logger.warning(
                    f"AI selected same-artist back-to-back (attempt {attempt}/{max_reselect_attempts}): "
                    f"{selected_artist}. Retrying."
                )
                rejected_titles.add(selected_key)
                rejected_artists.setdefault(selected_artist_lower, selected_artist.strip())
            ...
            cooldown_default = 3
            max_recent_default = 2
            try:
                artist_cooldown_tracks = int(
                    bundle.get_agent_setting("track_selector", "artist_cooldown_tracks", cooldown_default)
                )
            except (TypeError, ValueError):
                artist_cooldown_tracks = cooldown_default
            try:
                max_recent_artist_plays = int(
                    bundle.get_agent_setting("track_selector", "max_recent_artist_plays", max_recent_default)
                )
            except (TypeError, ValueError):
                max_recent_artist_plays = max_recent_default

            artist_cooldown_tracks = max(0, artist_cooldown_tracks)
            max_recent_artist_plays = max(1, max_recent_artist_plays)

            recent_artists_lower = [a.lower() for a in recent_artists if a]
            recent_artist_counts = Counter(recent_artists_lower)
            recent_artist_cooldown = recent_artists_lower[:artist_cooldown_tracks] if artist_cooldown_tracks else []

            if recent_artist_cooldown and selected_artist_lower in recent_artist_cooldown:
                logger.warning(
                    f"AI selected cooldown artist (attempt {attempt}/{max_reselect_attempts}): "
                    f"{selected_artist}. Retrying."
                )
                rejected_titles.add(selected_key)
                rejected_artists.setdefault(selected_artist_lower, selected_artist.strip())
            ...
            if recent_artist_counts.get(selected_artist_lower, 0) >= max_recent_artist_plays:
                logger.warning(
                    f"AI selected overplayed artist (attempt {attempt}/{max_reselect_attempts}): "
                    f"{selected_artist} played {recent_artist_counts.get(selected_artist_lower)}x recently. Retrying."
                )
                rejected_titles.add(selected_key)
                rejected_artists.setdefault(selected_artist_lower, selected_artist.strip())
                selection = None
        ...
        if not selection or not selected_artist or not selected_title:
            metrics.record_local_library_fallback()
            return await select_track(db, bundle, state, prev_song, history_ids)
        logger.info(f"🎵 AI selected: {selected_artist} - {selected_title}")
        ...
        intent = TrackIntent(
            ...
            album=None,  # Will be filled during acquisition if available
            isrc=None,  # Will be filled during acquisition if available
        ...
        if success:
            ...
            # Load the acquired song with features eagerly loaded
            result = await db.execute(
                select(Song)
                .options(selectinload(Song.features))
                .where(Song.uuid == intent.acquired_song_uuid)
            )
            ...
            if song:
                ...
                return {
                    ...
                    "artwork_url": song.artwork_url,  # Include artwork for frontend display
                    "features": {
            ...
            # Add normalized title for failed track
            normalized_failed = _normalize_song_title(intent.title)
            if normalized_failed:
                excluded_normalized.add(normalized_failed)
            
            for track in recent_catalog_tracks:
                excluded.add(f"{track.artist.lower()}|{track.title.lower()}")
                # Also add normalized title
                normalized_title = _normalize_song_title(track.title)
                if normalized_title:
            ...
            if scored_tracks and len(scored_tracks) > 0:
                ...
                if success:
                    ...
                    result = await db.execute(
                        select(Song)
                        .options(selectinload(Song.features))
                        .where(Song.uuid == fallback_intent.acquired_song_uuid)
                    )
                    song = result.scalar_one_or_none()
...
async def plan_transition(
    ...
    # Default fallback plan (with new fields)
    song_a_duration = song_a.get("duration_sec") or song_a.get("duration") or 180.0
    song_b_duration = song_b.get("duration_sec") or song_b.get("duration") or 180.0
    
    default_plan = {
        "transition_type": "crossfade",
        "mix_length_bars": 16,
        "start_position_a_seconds": max(0, song_a_duration - 20.0),  # Last 20s of Song A
        "start_position_b_seconds": max(5.0, min(8.0, song_b_duration * 0.1)),  # 5-8s or 10% of duration
    ...
    try:
        logger.info("Transition planner using direct JSON output")
...
async def write_intro_speech(
    ...
    Args:
        ...
        use_tools: Whether to use database tools for context (default True)
    ...
    try:
        ...
        # Pass tools to LLM so it can call them via tool calling
        tools_to_use = None
        if use_tools:
            from backend_v2.integrations.db_tools import SPEECH_TOOLS
            tools_to_use = SPEECH_TOOLS
            logger.info(f"🔧 Enabling {len(SPEECH_TOOLS)} tools for intro speech (including Deezer similarity)")
        ...
        result = await client.generate_dj_intro_speech(
            ...
            auto_log=True,
            tools=tools_to_use,  # Pass tools for LLM tool calling
        )
...
async def write_transition_speech(
    ...
    Uses persona cooldowns to prevent repeating roast topics too frequently.
    Enhanced with Spotify listening data for personalized references.
    ...
    # Check if next song is from user's Spotify top tracks
    next_song = context.get('next_song', {})
    spotify_hook = get_spotify_track_context(next_song, bundle)
    if spotify_hook:
        context['spotify_hook'] = spotify_hook
        logger.info(f"🎵 Spotify hook detected: {spotify_hook['title']} (rank #{spotify_hook['rank']} in {spotify_hook['time_range']})")
    
    # Tools will be passed to LLM for tool calling (LLM decides which tools to use)
    # No direct tool calls - let the LLM use tools via tool calling
    
    try:
        # Merge banter history with persona do_not_repeat and DJ history
        combined_history = list(banter_history or [])
        combined_history.extend(do_not_repeat)
        
        # Add recent DJ speeches to avoid repetition
        if bundle.dj_history.recent_speeches:
            combined_history.extend(bundle.dj_history.recent_speeches[:5])
        ...
        if use_tools:
            from backend_v2.integrations.db_tools import SPEECH_TOOLS
            tools_to_use = SPEECH_TOOLS
            logger.info(f"🔧 Enabling {len(SPEECH_TOOLS)} tools for transition speech (including Deezer similarity)")
...
async def persist_segment(
    ...
    from backend_v2.models.spotify_context import DJSpeechHistory
    import json
    import re
    ...
    # NEW: Persist DJ speech history if TTS was used
    if banter_text and tts_used:
        # Extract mentioned artists and tracks from banter
        mentioned_artists = []
        mentioned_tracks = []
        
        # Simple extraction: look for patterns like "Artist - Track" or "Track by Artist"
        ...
        if song_uuid:
            ...
            if song and song.artist:
                mentioned_artists.append(song.artist)
            if song and song.title:
                mentioned_tracks.append(song.title)
        
        # Also try to extract from speech text (basic pattern matching)
        # Look for quoted strings or capitalized phrases
        quoted_pattern = r'"([^"]+)"'
        quoted_matches = re.findall(quoted_pattern, banter_text)
        for match in quoted_matches:
            if len(match) > 3:  # Avoid short matches
                mentioned_tracks.append(match)
        
        dj_speech = DJSpeechHistory(
            user_id=user_id,
            session_id=session_id,
            speech_text=banter_text,
            mentioned_artists=json.dumps(mentioned_artists),
            mentioned_tracks=json.dumps(mentioned_tracks),
        )
        db.add(dj_speech)
        await db.flush()
        
        # Invalidate preference bundle cache to include new speech history
        get_bundle_cache().invalidate(user_id)
        
        logger.debug(f"Persisted DJ speech history: {len(mentioned_artists)} artists, {len(mentioned_tracks)} tracks")
...
def should_dj_speak(
    ...
    if is_intro:
        logger.debug(f"should_dj_speak: Intro segment, returning True")
        return True
    ...
    should_speak = songs_since_last_speech >= threshold
    
    logger.info(
        f"🎤 should_dj_speak: personality={personality}, "
        f"songs_since_last_speech={songs_since_last_speech}, "
        f"threshold={threshold}, should_speak={should_speak}"
    )
