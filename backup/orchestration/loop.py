...
from backend_v2.config import SEGMENT_DIR, SONG_CACHE_DIR, USE_LANGGRAPH_V3
...
class DJLoop:
    ...
    async def _run(self):
        ...
        # Log which orchestration mode is active
        if USE_LANGGRAPH_V3:
            logger.info(f"🎧 DJLoop running for user {self.user_id} [LangGraph v3 MODE]")
        else:
            logger.info(f"🎧 DJLoop running for user {self.user_id} [v2 MODE]")
        ...
        try:
            ...
            while not self._shutdown:
                try:
                    ...
                    # =========================================================
                    # LangGraph v3 Path (if enabled)
                    # =========================================================
                    if USE_LANGGRAPH_V3:
                        await self._produce_segment_v3()
    ...
    async def _produce_segment_v3(self):
        """
        Produce a segment using LangGraph v3 orchestration.
        
        This method uses the invoke_segment() API from the LangGraph runtime,
        which handles:
        - Mood loading and preferences
        - Track selection (with tool calling)
        - Acquisition (download if needed)
        - Transition planning
        ...
        # Determine if this is initial segment
        is_initial = not self.current_song
        is_resuming = bool(self.resume_song_uuid and self.resume_position_sec and self.resume_position_sec > 0)
        
        logger.info(f"🎧 [v3] Producing segment for user {self.user_id} "
                   f"(initial={is_initial}, resuming={is_resuming})")
        ...
        await emitter.emit_status(
            ...
            user_message="Your AI DJ is creating the next segment...",
            session_id=self.session_id,
            debug_message="Using LangGraph v3 orchestration",
        )
        
        try:
            # Call the LangGraph v3 invoke_segment API
            # This runs the full supervisor graph and returns a segment dict
            segment_meta = await invoke_segment(
                user_id=self.user_id,
                session_id=self.session_id,
                mood_id=self.mood_id,
                # Pass resume info for initial segment
                resume_song_uuid=self.resume_song_uuid if is_resuming else None,
                resume_position_sec=self.resume_position_sec if is_resuming else None,
                # Pass current song info for transitions
            ...
            if not segment_meta:
                logger.error(f"[v3] invoke_segment returned None for user {self.user_id}")
                await emitter.emit_status(
                    user_id=self.user_id,
                    category=StatusCategory.GENERATION,
                    step=StatusStep.PLANNING,
                    severity=Severity.ERROR,
                    user_message="Failed to generate segment. Retrying...",
                    session_id=self.session_id,
                )
                return
            ...
            if not path or not os.path.exists(path):
                logger.error(f"[v3] Segment path missing or invalid: {path}")
                return
            
            # Extract song info from segment
            song_uuid = segment_meta.get("song_uuid", "")
            song_title = segment_meta.get("title", "Unknown")
            song_artist = segment_meta.get("artist", "Unknown")
            song_artwork = segment_meta.get("artwork_url")
            song_duration = segment_meta.get("duration", 0)
            
            # Update loop state
            if song_uuid and song_uuid not in self.songs_played:
                self.songs_played.append(song_uuid)
            
            # Update current song for next iteration
            self.current_song = {
                "uuid": song_uuid,
                "title": song_title,
                "artist": song_artist,
                "artwork_url": song_artwork,
                "local_path": segment_meta.get("local_path", path),
            ...
            await self._update_session_current_track(self.current_song)
            ...
            # Emit segment ready for client synchronization
            ...
            logger.info(f"🎧 [v3] Queued segment {self.segments_produced}: "
                       f"{song_artist} - {song_title}")
            
        except Exception as e:
            logger.error(f"[v3] Segment production failed for user {self.user_id}: {e}")
            import traceback
            traceback.print_exc()
            ...
            await emitter.emit_status(
                ...
                step=StatusStep.PLANNING,
                severity=Severity.ERROR,
                user_message="Something went wrong. Retrying...",
                session_id=self.session_id,
                debug_message=str(e),
            )
    ...
    async def _produce_initial_segment(self):
        ...
        try:
            ...
            # Use pre-generated intros for the FIRST time each mood is played
            # Check if THIS SPECIFIC MOOD has been played before (not global history)
            mood_has_play_history = False
            if bundle.mood:
                ...
                async with get_db_session() as db:
                    result = await db.execute(
                        ...
                        .where(
                            PlayHistory.user_id == self.user_id,
                            PlayHistory.mood_id == bundle.mood.id
                        )
                    )
                    mood_play_count = result.scalar_one()
                    mood_has_play_history = mood_play_count > 0
            
            if (bundle.mood and bundle.mood.intro_segment_path and 
                not self.intro_already_used and 
                not mood_has_play_history):  # Skip if THIS MOOD has been played before
                intro_path = bundle.mood.intro_segment_path
                if os.path.exists(intro_path):
                    logger.info(f"Using pre-generated intro for mood '{bundle.mood.name}' (first time playing this mood): {intro_path}")
                    ...
                    return
            elif mood_has_play_history and bundle.mood and bundle.mood.intro_segment_path:
                logger.info(f"Skipping pre-generated intro for mood '{bundle.mood.name}' (mood has been played before) - generating fresh intro")
