...
class UserProfile(Base):
    ...
    energy_preference: Mapped[Optional[str]] = mapped_column(
        String(20),
        nullable=True,
        default=None
    )  # "high_energy" | "low_energy" | "mixed" | None
    
    tempo_preference: Mapped[Optional[str]] = mapped_column(
        String(20),
        nullable=True,
        default=None
    )  # "fast" | "slow" | "mixed" | None
    
    # Listening contexts (JSON array: ["workout", "focus", "party", "relaxing", "commute", etc.])
    listening_contexts: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    
    # Era preference for track selection
    era_preference: Mapped[Optional[str]] = mapped_column(
        String(20),
        nullable=True,
        default="mixed"
    )  # "new_releases" | "classics" | "mixed" | "no_preference"
