...
class Session(Base):
    ...
    playback_position_sec: Mapped[float] = mapped_column(Float, default=0.0, nullable=False)
    last_heartbeat_at: Mapped[Optional[str]] = mapped_column(String(50), nullable=True)
    is_active: Mapped[int] = mapped_column(Integer, default=1, nullable=False)  # 1=active, 0=stopped
    current_song_uuid: Mapped[Optional[str]] = mapped_column(String(36), nullable=True)
    ...
    current_song_artwork: Mapped[Optional[str]] = mapped_column(String(500), nullable=True)
...
class ToolUsageLog(Base):
    """Tool execution log for AI agent tool calls.
    
    Stores all tool executions with inputs and outputs for debugging and analysis.
    """
    ...
    llm_trace_id: Mapped[Optional[int]] = mapped_column(
        Integer,
        ForeignKey("llm_trace.id", ondelete="SET NULL"),
        nullable=True,
        index=True
    )
    agent_name: Mapped[Optional[str]] = mapped_column(String(50), nullable=True)
    tool_name: Mapped[str] = mapped_column(String(100), nullable=False)
    tool_arguments: Mapped[Optional[str]] = mapped_column(Text, nullable=True)  # JSON
    tool_result: Mapped[Optional[str]] = mapped_column(Text, nullable=True)  # JSON
    execution_time_ms: Mapped[Optional[float]] = mapped_column(Float, nullable=True)
    success: Mapped[bool] = mapped_column(Integer, default=1, nullable=False)  # 1 = success, 0 = failure
    error_message: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    created_at: Mapped[Optional[str]] = mapped_column(String(50), nullable=True)
    
    def __repr__(self) -> str:
        return f"<ToolUsageLog(id={self.id}, tool_name={self.tool_name}, success={self.success})>"