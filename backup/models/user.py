...
class User(Base):
    ...
    spotify_connected_at: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True),
        nullable=True,
        default=None,
    )
    ...
    spotify_context: Mapped[Optional["SpotifyUserContext"]] = relationship(
        "SpotifyUserContext",
        back_populates="user",
        uselist=False,
        cascade="all, delete-orphan"
    )
    dj_speech_history: Mapped[List["DJSpeechHistory"]] = relationship(
        "DJSpeechHistory",
        back_populates="user",
        cascade="all, delete-orphan"
    )
